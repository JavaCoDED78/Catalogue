...
class ProductReviewsRestControllerTest {
    ...
    @Test
    void receiveProductReviewsByProductId_ReturnsProductReviews() {
        // given
        doReturn(Flux.fromIterable(List.of(
                ...
                new ProductReview(UUID.fromString("be77f95a-cb05-11ee-91a3-1bdc94fa9de4"), 1, 5,
                        "Отзыв №3", "user-3")
        ))).when(productReviewsService).getProductReviewsByProduct(1);

        // when
        StepVerifier.create(productReviewsRestController.receiveProductReviewsByProductId(1))
                // then
                .expectNext(
                        new ProductReview(UUID.fromString("bd7779c2-cb05-11ee-b5f3-df46a1249898"), 1, 1,
                                "Отзыв №1", "user-1"),
        ...
        verify(productReviewsService).getProductReviewsByProduct(1);
        verifyNoMoreInteractions(productReviewsService);
    }
    ...
    void createProductReview_ReturnsCreatedProductReview() {
        ...
        doReturn(Mono.just(new ProductReview(UUID.fromString("5a9ba234-cbd6-11ee-acab-5748ca6678b9"), 1, 4,
                ...
                .when(productReviewsService)
                .createProductReview(1, 4, "В целом норм", "5f1d5cf8-cbd6-11ee-9579-cf24d050b47c");

        // when
        StepVerifier.create(productReviewsRestController.createProductReview(
